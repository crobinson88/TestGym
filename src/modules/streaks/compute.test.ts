...
describe("US public holidays on the work streaks", () => {
  // Thanksgiving 2026 is Thursday the 26th; the 27th is an ordinary Friday.
  const THANKSGIVING = "2026-11-26";
  const FRIDAY = "2026-11-27";

  const workStreak = (days: Iterable<string>, today: string) =>
    buildStreaks({
      today,
      french: new Set(),
      gym: new Set(),
  ...
  it("bridges a holiday nothing was logged on", () => {
    // Worked the three days before Thanksgiving and again on the Friday.
    const days = [...run("2026-11-25", 3), FRIDAY];
    expect(workStreak(days, FRIDAY).current).toBe(4);
  });
  ...
  it("still breaks on an ordinary missed day", () => {
    // Nothing on Friday the 27th, so by Saturday the run is over.
    expect(workStreak(run("2026-11-25", 3), "2026-11-28").current).toBe(0);
  });

  it("bridges a holiday in the all-time best run too", () => {
    const days = [...run("2026-11-25", 3), FRIDAY];
    expect(workStreak(days, "2027-03-01").best).toBe(4);
  });

  it("leaves the personal streaks counting holidays", () => {
    const s = buildStreaks({
      today: THANKSGIVING,
      french: new Set(),
      gym: new Set(run("2026-11-25", 3)),
      tgm: new Set(),
      getbuddy: new Set(),
      smoke_free: new Set(),
    }).find((x) => x.key === "gym")!;
