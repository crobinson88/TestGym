...
- `stocks(id uuid pk, ticker text, name text, notes text, links jsonb default '[]', documents jsonb default '[]', client_id uuid, user_id uuid default auth.uid(), created_at, updated_at, deleted_at)` — per-ticker record holding the "general notes" / running thesis for a stock (one row per ticker, deduped client-side via `ensureStock`). `links` are URLs, `documents` are `{path,name,mediaType}` objects in the `share-images` bucket. The Stock page's Research library lists these plus the links/images attached to that ticker's trades. Same sync rules. Surfaced at `/shares/stock/:ticker`.
